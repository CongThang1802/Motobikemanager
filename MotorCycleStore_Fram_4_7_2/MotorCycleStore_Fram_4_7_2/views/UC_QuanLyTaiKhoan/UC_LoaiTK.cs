...
{
    public partial class UC_LoaiTK : UserControl
    {
        private bool status;
        public UC_LoaiTK()
        {
        ...
        {
            dgv_Content.DataSource = Data_Entity.LoaiTK_Data.Get_LoaiTkList();
            edt_Ma.Text = "";
            edt_Ten.Text = "";
            edt_ChuThich.Text = "";
            ...
            if (e.RowIndex > -1)
            {
                switch (e.ColumnIndex)
                {
                    ...
                    case 1:
                        ...
                        {
                            Data_Entity.LoaiTK_Data.Delete_LoaiTK(this.dgv_Content.Rows[e.RowIndex].Cells[2].Value.ToString());
                            RefeshForm();
                        }
                        break;
                }
        ...
        {
            string id = Create_ID.Gennaral_ID(Data_Entity.LoaiTK_Data.Get_IdLoaiTk());
            edt_Ma.Text = id;
            edt_Ten.Text = "";
        ...
        {
            dgv_Content.DataSource = Data_Entity.LoaiTK_Data.Search_LoaiTkList(edt_Search.Text.Trim());
        }
        ...
        private void btn_Update_Click(object sender, EventArgs e)
        {
            if (this.status)
            {
                ...
                {
                    Data_Entity.LoaiTK_Data.Add_LoaiTK(new Data_Entity.Loai_TK()
                    {
                        id_LoaiTK = edt_Ma.Text.Trim(),
                        name_LoaiTK = edt_Ten.Text,
                        descrip_LoaiTK = edt_ChuThich.Text
                    });
                    RefeshForm();
                }
            }
            else
            {
                if (edt_Ten.Text.Trim() != "" && edt_ChuThich.Text.Trim() != "")
                {
                    Data_Entity.LoaiTK_Data.Edit_LoaiTK(new Data_Entity.Loai_TK()
                    {
                        id_LoaiTK = edt_Ma.Text.Trim(),
                        name_LoaiTK = edt_Ten.Text,
                        descrip_LoaiTK = edt_ChuThich.Text
